...
async function handleFileUpload(request, env) {
  ...
  try {
    ...
    if (fileName.endsWith('.xlsx') || fileName.endsWith('.xls')) {
      try {
        ...
        return Response.json({ 
          reply: `✅ קובץ Excel ${fileName} נשמר בהצלחה!\n\nהנתונים בקובץ:\n• מספר שורות נתונים: ${Object.keys(XLSX.read(buffer, { type: 'buffer' }).Sheets[workbook.SheetNames[0]]).length}\n• עמודות: ${workbook.SheetNames[0]}\n\nעכשיו תוכל לשאול אותי על הציונים והנתונים!` 
        });
    ...
    console.error(e);
    return Response.json({ reply: "שגיאה בשמירת הקובץ - " + e.message });
  }
}
